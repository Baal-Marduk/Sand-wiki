...
**Tech Stack:** Next.js 15 (App Router), TypeScript, Tailwind CSS, Prisma + PostgreSQL (hosted, e.g. Neon/Supabase), React Flow (tech-tree graph), Vitest (unit), Playwright + axe (e2e/a11y).

---

## Environment notes (Windows)

- **Database:** No Docker on this machine. We use a **hosted PostgreSQL** instance; the
  `DATABASE_URL` is supplied by the user and placed in `sand-wiki/.env` (gitignored). No
  `docker-compose.yml`.
- **Node on PATH:** Node was installed via winget after the shell session started, so freshly
  spawned PowerShell processes do **not** see `node`/`npm` until PATH is refreshed. **Every
  PowerShell command in this plan must be prefixed with:**

  ```powershell
  $env:Path = [Environment]::GetEnvironmentVariable("Path","Machine") + ";" + [Environment]::GetEnvironmentVariable("Path","User")
  ```

  Use the **PowerShell** tool, not Bash (the Bash environment has no Node at all).
...
sand-wiki/                      # app root (created in Task 1)
  .env.example / .env           # DATABASE_URL (hosted Postgres)       (Task 2)
  prisma/
...
## Task 2: Hosted Postgres environment

**Files:**
- Create: `sand-wiki/.env.example`
- Create: `sand-wiki/.env`

- [ ] **Step 1: Create `.env.example`** (placeholder, committed)

```
DATABASE_URL="postgresql://USER:PASSWORD@HOST:5432/DBNAME?sslmode=require"
```

- [ ] **Step 2: Create `.env`** with the **real hosted `DATABASE_URL`** supplied by the user
  (Neon/Supabase, etc.). It is gitignored by the repo-root `.gitignore`. Do not commit it.

- [ ] **Step 3: Verify connectivity**

Run (PATH-prefixed, from `sand-wiki/`): `npx prisma db execute --stdin` and pipe `SELECT 1;`
OR simply proceed — Task 3's `migrate dev` will fail clearly if the URL is wrong.
Expected: no connection error.

- [ ] **Step 4: Commit** (from repo root)

```bash
git add sand-wiki/.env.example
git commit -m "chore: add .env.example for hosted Postgres"
```
...
Run: `npm run db:seed`
Expected: seed reports 4 items / 2 tech nodes against the hosted Postgres.
...
> hosted Postgres instead of local Docker (no Docker on the dev machine).
