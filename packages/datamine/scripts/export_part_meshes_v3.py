...
Per part: ../../apps/wiki/public/meshes/<id>.bin
  layout (all per-triangle, non-indexed, T triangles):
    [T*9 f32 positions][T*9 i8 normals(/127)][T*6 f32 uvs][T*1 u8 texSlot]
  texSlot indexes into this part's local texture list (mesh_index[id].tex).
Shared albedo textures: ../../apps/wiki/public/meshes/tex/<n>.png  (downscaled to <=512px)
Index: ../data/generated/mesh_index.json  (i.e. packages/data/generated/mesh_index.json)
  { id: {t, b:[min,max], l:lod, tex:["meshes/tex/3.png", ...], col:[[r,g,b]...]} , _cell }
...
OUT = '../../apps/wiki/public/meshes'
TEXOUT = '../../apps/wiki/public/meshes/tex'
os.makedirs(OUT, exist_ok=True)
os.makedirs(TEXOUT, exist_ok=True)
MAXTEX = 512

parts = json.load(open('sek-out/parts_v2.json', encoding='utf-8'))['parts']
...
def resolve_texture(texco):
    ...
    try:
        ...
        return None
    tex_registry[tid] = {'slot': slot, 'file': f'meshes/tex/{fn}'}
    return slot
