...
Output: ../../apps/wiki/public/containers/<key>.png + sek-out/container_art.json
"""
import UnityPy, json, os, re, math, glob
import numpy as np
from PIL import Image, ImageDraw

# Game files: $GAMEFILES (an absolute StandaloneWindows64 dir, e.g. a SAND install) or
# the local gamefiles/ copy. Read-only/offline — only mine with the game NOT running.
BASE = os.environ.get('GAMEFILES') or os.path.join('gamefiles', 'Sand_Data', 'StreamingAssets', 'aa', 'StandaloneWindows64')
OUT = os.environ.get('OUT_DIR') or '../../apps/wiki/public/containers'
os.makedirs(OUT, exist_ok=True)

def bundle(name):
    """Resolve a bundle by name; the duplicateassetisolation hash changes per build."""
    p = os.path.join(BASE, name + '.bundle')
    if os.path.exists(p):
        return p
    hits = glob.glob(os.path.join(BASE, name + '_*.bundle'))
    if not hits:
        raise FileNotFoundError(f'bundle not found: {name}* in {BASE}')
    return hits[0]
...
env = UnityPy.load(bundle('views_assets_all'),
                   bundle('geometry_assets_all'),
                   bundle('duplicateassetisolation_assets_all'))
...
# The projection renders every container upside down (the asymmetric ones — cabinet,
# safe, ironclad — just made it obvious). We right ALL of them with a 180° rotation
# about the world X axis (y,z -> -y,-z) BEFORE the camera transform, so they come out
# upright AND keep the side-top camera. (A 2D image flip would instead invert the camera
# elevation -> "from below".) If a model ends up showing its back, swap the rotation to
# about Z instead: tris * [-1, -1, 1].
FLIP_AXIS = np.array([1.0, -1.0, -1.0])  # 180° about X
...
for name, root_name in SOURCE_VIEW.items():
    ...
    tris = tris * FLIP_AXIS  # right the inverted prefab in 3D, before the camera
...
json.dump(art, open('sek-out/container_art.json', 'w', encoding='utf-8'), indent=1)
