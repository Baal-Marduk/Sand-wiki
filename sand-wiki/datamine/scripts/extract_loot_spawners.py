...
Output: <datamine>/extracted/json/entity_loot.json
Game files: <datamine>/gamefiles/... by default, or set $GAMEFILES to an absolute
StandaloneWindows64 dir (e.g. an external SAND install copy).
"""
...
BASE = os.environ.get("GAMEFILES") or os.path.join(DM, "gamefiles", "Sand_Data", "StreamingAssets", "aa", "StandaloneWindows64")
BASE = os.path.join(BASE, "")  # ensure trailing separator for BASE + bundle concatenation
...
json.dump(result, open(os.path.join(EXT, 'entity_loot.json'), 'w', encoding='utf-8'), indent=1)
json.dump(walker_stats, open(os.path.join(EXT, 'walker_part_stats.json'), 'w', encoding='utf-8'), indent=1)
