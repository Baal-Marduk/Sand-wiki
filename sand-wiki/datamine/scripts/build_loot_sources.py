"""Build source-level loot data for the loot pipeline's container catalog.
...
Input:  extracted/json/{entity_loot,loottables_voyage,loottables_storm}.json
Output: data/loot_sources.json  (consumed by build_container_loot.py)
"""
import json, re, os
from collections import defaultdict

DM = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # sand-wiki/datamine
EXT = os.path.join(DM, "extracted", "json")

entity_loot = json.load(open(os.path.join(EXT, 'entity_loot.json'), encoding='utf-8'))
voy = json.load(open(os.path.join(EXT, 'loottables_voyage.json'), encoding='utf-8'))['_lootTables']['$items']
sto = json.load(open(os.path.join(EXT, 'loottables_storm.json'), encoding='utf-8'))['_lootTables']['$items']
...
json.dump(result, open(os.path.join(DM, 'data', 'loot_sources.json'), 'w', encoding='utf-8'), indent=1)
for s in result:
