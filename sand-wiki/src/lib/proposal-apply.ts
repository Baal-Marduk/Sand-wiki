...
export async function applyProposal(proposalId: string, reviewerSteamId: string): Promise<void> {
  await prisma.$transaction(async (tx) => {
    ...
    if (p.targetType === "item") await tx.item.update({ where: { slug: p.targetSlug }, data: update as any });
    else if (p.targetType === "envEntity") await tx.envEntity.update({ where: { slug: p.targetSlug }, data: update as any });
    else if (p.targetType === "tramplerPart") await tx.tramplerPart.update({ where: { slug: p.targetSlug }, data: update as any });
